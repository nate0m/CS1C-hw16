...
// Q#1
// The type requirements for the input iterator would be it to be a type that supports
// the increment (++) operator and the dereference (*) operator. The increment operator
// is used to go to the next value of the container and the dereference is to get the 
// value of that container location. The dereferenced value would have a type 
// requirement of the comparison operators (==, !=) which allows for it to be compared
// to the comaprable.
//
// The type requirements for the Equality Comparable would be a type that supports the
// boolean comparison operators (==, !=). The comparison operators are used to check
// if the comparable is equal to the iterator. 
//
// It is important to note that if a custom type is used than these operators would all
// have to be overloaded for either the iterator type requirement or the comparable type
// requirement depending on which one it is.
